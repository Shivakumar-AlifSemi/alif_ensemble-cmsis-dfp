 ...
 * @brief    Application-facing ISP parameter structs for runtime get/set.
 *           These are passed via ISP_Control() using ISP_CONTROL_SET_PARAM
 *           and ISP_CONTROL_GET_PARAM.  No ISP library headers are required
 *           by the application.
 ******************************************************************************/

#ifndef ISP_CTRL_PARAMS_H_
#define ISP_CTRL_PARAMS_H_
...
/** @brief Cached AE sensor values (int_time, again, dgain). */
struct isp_ae_cached_values {
    uint32_t int_line; /**< Integration time in lines */
...
/** @brief AWB white-pixel curve (16-point). */
 ...
 * Passed alongside the other module params.
...
/** @brief Binning parameters. */
struct isp_binning_param {
    /** 0: disable binning, 1: enable */
    uint8_t enable;
    /** Horizontal binning step size [0-255] */
    uint8_t h_step;
    /** Vertical binning step size [0-255] */
    uint8_t v_step;
};
 ...
 *
 * Set the corresponding bit before calling ISP_Control() to indicate
 * which module sub-structs are valid.
 */
...
#define ISP_PARAM_MASK_BINNING   (1U << 11)
#define ISP_PARAM_MASK_ALL        (0xFFFU)

/**
 * @brief Single aggregate struct passed to ISP_CONTROL_SET_PARAM /
 *        ISP_CONTROL_GET_PARAM.
 *
